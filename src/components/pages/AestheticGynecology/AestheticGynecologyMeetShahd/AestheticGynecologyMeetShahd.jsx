...
const AestheticGynecologyMeetShahd = ({ data }) => {
  ...
  const title = data?.title || t("Compassionate Care");
  const subtitle = data?.subtitle || t("Meet Dr Shahd");
  const description = data?.description || t("Meet Shahd Desc");
  const portraitImage =
    data?.main_image_url ||
    (data?.image
  ...
  const services = [
    {
      ...
      description:
        "Addressing dryness, discomfort, and overall tissue quality for better intimate wellness.",
      icon: "https://img.icons8.com/ios/100/ddb2b5/uterus.webp",
    },
    {
      title: "TISSUE REJUVENATION",
      description:
        "Enhancing structural integrity and aesthetics through advanced regenerative techniques.",
      icon: "https://img.icons8.com/ios/100/ddb2b5/syringe.webp",
    },
    {
      title: "FUNCTIONAL COMFORT",
      description:
        "Improving daily comfort and confidence with specialized aesthetic treatments.",
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(180deg, rgba(255, 255, 255, 0.37) 0%, rgba(221, 178, 181, 0.37) 50%, rgba(255, 255, 255, 0.37) 100%)",
      }}
