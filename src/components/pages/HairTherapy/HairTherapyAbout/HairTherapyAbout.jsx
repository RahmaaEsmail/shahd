...
export default function HairTherapyAbout({ data }) {
  ...
  const subtitle = data?.ht_about_main?.subtitle || t("About The Surgery");
  const title = data?.ht_about_main?.title || t("What is hair transplant?");
  const mainImage =
    data?.main_image ||
    "/SHAHD-IMAGE/hair-therapy/Image_fx - 2026-02-04T001320.240 1.webp";
  ...
  const aboutCards =
    data?.cards && data.cards.length > 0
      ? data.cards.map((item, idx) => ({
          id: item.id,
          title: item?.title,
          img: `/SHAHD-IMAGE/hair-therapy/Frame 1000005833 (${idx + 1}).webp`,
        }))
      : [
          {
            id: 1,
  ...
  return (
    <motion.section
      ...
      style={{
        background:
          "linear-gradient(180deg, #FFFFFF 0%, #F3E5E6 50%, #FFFFFF 100%)",
      }}
      initial="hidden"
