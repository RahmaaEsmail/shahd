...
export default function HairTherapyBeforeLeftContent({ activeIndex, setActiveIndex, activeTreatment, treatments }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: false }}
      transition={{ duration: 0.8, delay: 0.2 }}
      className="md:col-span-12 lg:col-span-5 flex flex-col gap-6  xl:max-h-[700px] xl:overflow-y-auto no-scrollbar xl:pr-4"
    >
      {/* Treatment Info */}
      <AnimatePresence mode="wait">
        <motion.div
      ...
      {/* Thumbnail Strip */}
      <div className="flex gap-4 overflow-x-auto no-scrollbar h-full  py-5 justify-center lg:justify-stretch">
        {treatments.map((treatment, index) => (
          <motion.button
            key={treatment.id}
            onClick={() => handleThumbnailClick(index)}
          ...
          >
            <div className={`relative w-16  h-16 md:w-20 md:h-20 lg:w-24 lg:h-full overflow-hidden rounded-full transition-all duration-300 
