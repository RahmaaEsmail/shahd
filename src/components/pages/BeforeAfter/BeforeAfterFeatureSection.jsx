...
export default function BeforeAfterFeatureSection({
  activeCase,
  setActiveCase,
  CASES,
}) {
    ...
    <section className="max-w-7xl mx-auto px-6 py-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-8">
        <motion.div
          ...
          viewport={{ once: false }}
        >
          <p className="text-[16px] tracking-[4px] uppercase text-secondary font-bold mb-3">
            {t("Portfolio")}
          </p>
          <h2 className="font-main text-5xl font-normal text-primary">
            {t("Transformations")}
          </h2>
        </motion.div>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="font-serif text-xl text-primary/40">
            ...
            <small className="text-sm">/ {CASES.length}</small>
          </span>
          <button
            onClick={() =>
              setActiveCase((prev) =>
